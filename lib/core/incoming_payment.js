                    ...
                    if (array) {
                        ...
                        { //忽略外出的交易
                            if (_this.payment.direction == "outgoing") {
                                ...
                                gatewayd.logger.info('code:' + json['code']);
                                gatewayd.logger.info('name:' + json['name']);
                                gatewayd.logger.info('symbol:' + json['symbol']);
                                gatewayd.logger.info('amount:' + json['amount']);

                                //资产创设TRUSTLINE类型
                                if (array[0].parsed_memo_type.toUpperCase() == 'TRUSTLINE') {
                                    gatewayd.logger.info('code is exist?');
                                    gatewayd.data.assets.read({ code: json['code'] }, function (err, asset) {
                                        console.log('error:' + err);
                                        if (!err) {
                                            if (asset) {
                                                gatewayd.logger.info('code is exist:' + asset);
                                                var param = '{"response":"error","reason":"code exist"}';
                                                response_msg(param, payment.source_account);
                                                ...
                                                gatewayd.data.assets.create(record, function (err, response) {
                                                    ...
                                                    if (!err) {
                                                        var param = '"code":"' + json['code'] + '","name":"' + json['name'] + '",symbol":"' + json['symbol'] + '","amount":"' + json['amount'] + '","hash":"' + payment.hash + '"}';
                                                        console.log(param);
                                                        response_msg(param, payment.source_account);

                                                    }
                                                    else {
                                                        var param = '{"response":"error","reason":"insert error"}';
                                                        response_msg(param, payment.source_account);
                                                    }
