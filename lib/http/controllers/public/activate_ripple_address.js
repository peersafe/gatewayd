...
module.exports = function(req, res) {
    ...
    if (activatedRippleAddress) {
        ...
        total_amount: activatedRippleAddress.total_amount + config.get('ACTIVATE_ADDRESS').amount;
      };
    	...
    	gateway.api.sendPayment(1, "XRP", address, function(err, response) {
	    	if (err) {
	    		...
	    		addressOpts.total_amount = addressOpts.total_amount - config.get('ACTIVATE_ADDRESS').amount;
	    		gateway.data.activatedRippleAddress.update(addressOpts, function(err, model) {});
        ...
        address: address,
        enabled: false,
        total_amount: config.get('ACTIVATE_ADDRESS').amount
      };
