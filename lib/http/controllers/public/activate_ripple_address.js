...
module.exports = function(req, res) {
    ...
    var amount = gateway.config.get('ACTIVATE_ADDRESS').amount;
    ...
    if (activatedRippleAddress) {
    	...
    	var addressOpts = {
            ...
            total_amount: activatedRippleAddress.total_amount + amount
          };
    	gateway.data.activatedRippleAddress.update(addressOpts, function(err, model) {});
    	gateway.api.sendPayment(amount, "XRP", address, function(err, response) {
	    	if (err) {
	    		addressOpts.enabled = true;
	    		addressOpts.total_amount = addressOpts.total_amount - amount;
	    		gateway.data.activatedRippleAddress.update(addressOpts, function(err, model) {});
